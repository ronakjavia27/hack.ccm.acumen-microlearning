...
# =====================================================================
# 🎨 TAILORED BEIGE & LIGHT BROWN RESPONSIVE CSS DECK
# =====================================================================
st.markdown('''
<style>
    /* Absolute background canvas defaults */
    html, body, [data-testid="stAppViewContainer"] {
        background-color: #F5F5DC !important;
    }
    
    /* Global clear text force matching */
    p, li, span, label, h1, h2, h3 {
    ...
    /* 🤎 LIGHT BROWN PANEL CONTRAST FOR DROPDOWNS & FILTER SHELLS */
    [data-testid="stVerticalBlock"] > div:has(.filter-container),
    .filter-container {
    ...
    /* Crisp Document Viewer Paper Card */
    .white-paper-card {
    ...
    /* Force visibility and styling of native input fields */
    ...
    /* Style hyperlinks inside the header */
    .nav-anchor {
    ...
    /* Mobile media stacking adaptations */
...
if df.empty:
    ...
    # 2. Dual Column Responsive Split (Filters 33% Left, Summary 66% Right)
    col_filters, col_viewer = st.columns([1, 2], gap="large")

    # Left Control Panel Column Thread
    with col_filters:
        # Wrap inputs inside our custom Light Brown Container CSS anchor class
        st.markdown('<div class="filter-container">', unsafe_allow_html=True)
        st.markdown('<h3 style="margin-top:0; padding-top:0;">🔍 Filter Inventory</h3>', unsafe_allow_html=True)
        
        search_query = st.text_input("Search Titles", placeholder="Type keywords...", label_visibility="collapsed")
    ...
    # Right Content Sheet Column Thread
    with col_viewer:
        if selected_paper:
            ...
            with title_block:
                st.markdown(f'<h2 style="margin:0; padding:0;">📜 {target_row["Paper/Guideline Name"]}</h2>', unsafe_allow_html=True)
            with doi_block:
            ...
            # Metadata Info Tags Section
            st.markdown(f'''
                <div style="margin-top: 10px; margin-bottom: 15px;">
                    <span style="background-color:#EFF6FF; color:#1E40AF; padding:4px 10px; border-radius:12px; font-size:12px; font-weight:600; margin-right:8px;">🧬 {target_row['System']}</span>
                    <span style="background-color:#FAF5FF; color:#6B21A8; padding:4px 10px; border-radius:12px; font-size:12px; font-weight:600; margin-right:8px;">📖 {target_row['Journal Name']}</span>
                    <span style="background-color:#F1F5F9; color:#475569; padding:4px 10px; border-radius:12px; font-size:12px; font-weight:600;">📑 {target_row['Type of Article']}</span>
                </div>
            ''', unsafe_allow_html=True)

            # Overlaid Main Sheet Container block
            st.markdown('<div class="white-paper-card">', unsafe_allow_html=True)
            
            # Read local JSON content
